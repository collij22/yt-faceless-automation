#!/usr/bin/env python3
"""
n8n Workflow Deployment Script (Windows Compatible)
...
class N8nDeployer:
    ...
    def validate_api_connection(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                print("[SUCCESS] Successfully connected to n8n API")
                return True
            else:
                print(f"[ERROR] API connection failed: {response.status_code} - {response.text}")
                return False
    ...
    def validate_workflow_file(self, filepath: str) -> Tuple[bool, Optional[Dict], Optional[str]]:
        ...
        try:
            ...
            print(f"[VALID] Validated workflow: {workflow_data['name']}")
    ...
    def check_existing_workflow(self, workflow_name: str) -> Optional[str]:
        ...
        try:
            ...
            print(f"[ERROR] Error checking existing workflows: {e}")
            return None
    ...
    def deploy_workflow(self, workflow_data: Dict, filepath: str) -> WorkflowDeployment:
        ...
        try:
            ...
            if existing_id:
                print(f"[UPDATE] Workflow '{workflow_name}' already exists (ID: {existing_id})")
                ...
                print(f"[CREATE] Creating new workflow: {workflow_name}")
            ...
            print(f"[SUCCESS] Successfully deployed: {workflow_name} (ID: {deployment.workflow_id})")

        except requests.RequestException as e:
            deployment.status = "failed"
            deployment.error = f"Deployment failed: {e}"
            print(f"[ERROR] Failed to deploy {workflow_name}: {e}")
    ...
    def activate_workflow(self, workflow_id: str, workflow_name: str) -> bool:
        ...
        try:
            ...
            response.raise_for_status()
            print(f"[ACTIVE] Activated workflow: {workflow_name}")
            return True
        except requests.RequestException as e:
            print(f"[ERROR] Failed to activate {workflow_name}: {e}")
            return False

    def test_webhook(self, webhook_url: str, workflow_name: str) -> bool:
        """Test a webhook endpoint with a simple GET request."""
        if not webhook_url:
            print(f"[WARN] No webhook URL found for {workflow_name}")
            return False
        ...
        try:
            ...
            if response.status_code in [200, 405]:
                print(f"[TEST-OK] Webhook endpoint active: {webhook_url}")
                return True
            else:
                print(f"[TEST-WARN] Webhook test returned {response.status_code}: {webhook_url}")
                return False

        except requests.RequestException as e:
            print(f"[TEST-ERROR] Webhook test failed for {workflow_name}: {e}")
            return False
    ...
    def deploy_all_workflows(self) -> bool:
        ...
        for deployment in self.deployments:
            status_symbol = "[SUCCESS]" if deployment.status == "deployed" else "[FAILED]"
            print(f"{status_symbol} {deployment.filename}")
        ...
        if success_count == len(self.workflow_files):
            ...
            print(f"\n{len(self.workflow_files) - success_count} workflows failed to deploy")
            return False
    ...
    def print_webhook_urls(self):
        ...
        if webhook_deployments:
            ...
            for deployment in webhook_deployments:
                ...
                print(f"N8N_{workflow_name.upper()}_WEBHOOK_URL={deployment.webhook_url}")
    ...
    try:
        ...
        try:
            ...
            if os.path.exists(".env"):
                ...
                print("No .env file found. Make sure N8N_API_KEY and N8N_API_URL are set.")
        except ImportError:
